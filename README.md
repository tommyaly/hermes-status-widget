...
- Hermes 是否在线
- Agent 今天服务了多久，Ai牛马有没有干满八小时？
- 当前或最近活跃的 Agent 会话（最多 3 个）
- 最近 24 小时用了多少 token
- 不同模型分别消耗了多少
- 菜单栏里的详细状态
- 桌面小组件里的简洁状态
...
1. 打开右侧或下方的 **Releases**。
2. 下载 `Hermes-Status-Widget-0.4.3.pkg`。
3. 双击安装。
...
为了让菜单栏和桌面小组件不挤在一起，当前或最近活跃的会话最多显示 3 个。正在进行的会话会排在前面。
...
- 活跃会话来自 `sessions` 和 `messages`，最多取 3 条；正在进行的会话优先，其次按最近活动时间排序。
